...
let numberToValidate = process.argv[5]
...
;(async()=>{
    ...
    if( !validationTypes.includes(validationType) ){
        lm.log(`Invalid mode entered: ${process.argv[3]} Valid modes: ${validationTypes.join(', ')}`)
        return
    }

    // Validate index and number to generate.  
    if( isNaN(startIndex) || isNaN(numberToValidate) ){
    ...
    const addresses = await addGen.generate(numberToValidate,startIndex)
    ...
    let requestData = {
        ...
        endIndex:startIndex + numberToValidate
    }

    let responseData = {}

    try {
        remoteResponse = await requestManager.post(config.platform.routes.validateAddresses,requestData)
    ...
    if ( validationType === 'hash' ){
        ...
        if ( responseData.hash === undefined || responseData.hash.length !== 64 ){
            lm.log(`Remote platform request malformed. Expected hash got: ${JSON.stringify(responseData)}`,true,true)
            return
        }
        ...
        let validationHash = crypto.createHash('sha256').update(validationString).digest('hex')

        if ( validationHash !== responseData.hash ) {
            lm.log(`Platform ${coin} addresses for index range ${startIndex} to ${endIndex} do not match local addresses when compared by hash. Run address by address comparison to find inconsistencies.`,true,true)
        } else {
            lm.log(`Platform ${coin} deposit addresses match deterministic addresses for range ${startIndex} to ${endIndex} when compared by hash`,true,false)
        } 
    ...
    if ( validationType === 'address' ){
        ...
        if ( responseData.addresses === undefined || typeof responseData.addresses !== 'object' ){
            lm.log(`Remote platform request malformed. Expected array of addresses and index ( Example: {addresses:{index1:address1,index2:address2}} ) got: ${JSON.stringify(responseData)}`,true,true)
            return
        }
        ...
        addresses.forEach(address => {
            ...
            if ( remoteAddresses[index] !== address.address ) {
                inconsistencies += `Index ${index}: expected ${address.address}, platform had ${remoteAddresses[address.index]}, `
            }
        ...
        if ( inconsistencies.length > 0  ) {
            lm.log(`Platform ${coin} deposit address inconsistencies found for index range ${startIndex} to ${endIndex}. Inconsistencies: ${inconsistencies}`,true,true)
        } else {
            lm.log(`Platform ${coin} deposit addresses match deterministic addresses for range ${startIndex} to ${endIndex} when compared by address.`,true,false)
        } 
