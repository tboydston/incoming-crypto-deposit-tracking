...
(async () => {
  if (config === undefined) {
    console.log(
      "Coin must be defined. Example: node generateAddresses.js BTC show 0 1"
    );
  }
  ...
  if (!validModes.includes(mode)) {
    lm.log(
      `Invalid mode entered: ${process.argv[3]} Valid modes: ${validModes.join(
        ", "
      )}`
    );
    return;
  ...
  if (isNaN(startIndex) || isNaN(endIndex)) {
    lm.log(
      `Start index and end index must be numbers. Example: node generateAddresses.js BTC show 0 1`
    );
    return;
  }
  ...
  let addresses = await addGen.generate(
    parseInt(numberToGenerate),
    parseInt(startIndex)
  );
