...
signatureManager.sign = async (privKey, data) => {
  const result = {};
  const signer = crypto.createSign(signatureManager.algorithm);

  signer.update(data);

  result.signature = signer.sign(privKey, signatureManager.sigFormat);
  result.data = data;
...
signatureManager.verify = async (pubKey, data, sig) => {
  const verifier = crypto.createVerify(signatureManager.algorithm);
  verifier.update(data);

  return verifier.verify(pubKey, sig, signatureManager.sigFormat);
};
