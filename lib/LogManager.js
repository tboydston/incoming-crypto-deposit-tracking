...
class LogManager {
    ...
    async log(logContent,logToConsole=true,sendToTg=false){
        ...
        const tgContent = logContent.replace(/\./g,'\\.').replace(/\{/g,'\\{').replace(/\}/g,'\\}').replace(/\)/g,'\\)').replace(/\(/g,'\\(')
        ...
        if ( sendToTg ) {
            if ( this.serviceName === undefined ){
                this.tg.sendMessage(this.tgChatId,tgContent,{parse_mode: "MarkdownV2"})
            } else {
                this.tg.sendMessage(this.tgChatId,`${this.serviceName}: ${tgContent}`,{parse_mode: "MarkdownV2"})
            }
        }

        fs.appendFileSync(`${__dirname}/../logs/${this.logFolder}/${this.coin}-${this.logFolder}-log.txt`,`${timeStamp} | ${logContent}\n`)
